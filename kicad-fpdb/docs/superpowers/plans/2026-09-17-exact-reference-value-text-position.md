...
> **Post-execution correction:** every `0.94mm` (DIP/CERDIP `socket`) and `1.0mm` (R-AXIAL) value below was wrong — a regex DOTALL-crossing bug in the original investigation. The actual values are `0.745mm` and `0.87mm`, caught by Task 3's own regression run and corrected in the same commit. See the design spec's own correction note and `kicad_fpdb/footprint_diff.py`'s `KNOWN_TEXT_POSITION_ANOMALIES` comment for the full explanation. Left unedited below as the historical record of what was planned.
